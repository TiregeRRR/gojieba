...
public:
  ...
  KeywordExtractor(const DictTrie *dictTrie, const HMMModel *model,
                   const string &idfStr, const string &stopWordStr)
      : segment_(dictTrie, model) {
    LoadIdfDict(idfStr);
    LoadStopWordDict(stopWordStr);
  }
  ~KeywordExtractor() {}
...
private:
  void LoadIdfDict(const string &idfString) {
    istringstream ifs(idfString);
    string line;
    vector<string> buf;
    double idf = 0.0;
    double idfSum = 0.0;
  ...
  void LoadStopWordDict(const string &fileStr) {
    istringstream ifs(fileStr);
    string line;
